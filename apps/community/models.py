...
class Post(BaseModel):
    ...
    comment_nums = IntegerField(default=0, verbose_name="评论")

    is_excellent = BooleanField(default=0, verbose_name="是否为精华")
    is_hot = BooleanField(default=0, verbose_name="是否为热门")
    ...
    def extend(cls):
        ...
        return cls.select(cls, Post, relyed_user.id, relyed_user.nick_name, author.id, author.nick_name).join(
            Post, join_type=JOIN.LEFT_OUTER, on=cls.post).switch(cls).join(author, join_type=JOIN.LEFT_OUTER,
                                                                         on=cls.user).switch(cls).jonin(relyed_user,
